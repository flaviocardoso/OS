...
<div class="pesquisa text-center my-2">
	...
	<div class="collapse" id="collapsePesquisa">
		<div class="card card-body">
			...
			<div class="special go-botton mb-1">
				<div class="col-sm">
					<select ng-model="seach.dest_setor" ng-setor-resp ng-options="option as option for option in setordados">
						<option value="">Selecione</option>
					</select>
					<label>Setor</label>
				</div>
			...
			<div class="special go-botton mb-1">
				<div class="col-sm text-center">
					<select ng-model="seach.dest_area" ng-area-resp ng-options="option.valor as option.valor for option in areadados">
						<option value="">Selecione</option>
					</select>
					<label>Area</label>
				</div>
...
<div>
	<table class="table" style="overscroll-behavior-x: contain;">
		...
		<tbody>
			<tr style="background-color:{{ordem.cor_os}}" ng-repeat="ordem in $ctrl.verOs | filter: { solicitante : seach.solicitante} | filter: {data_in : seach.data_in} | filter: {data_up : seach.data_up} | filter: {dest_setor : seach.dest_setor || undefined} | filter: {dest_area : seach.dest_area || undefined} | filter:  {tecnico : seach.tecnico} | filter: {status : 'NOVA'}">
				<td class="text-center">
					<a ui-sref="dashboard.os.infoeditadalaudorespnova({ osID : ordem.id_os })" ui-sref-active="active" onclick="this.disabled=true;" class="btn btn-light">{{ordem.n_os}}</a>
				</td>
