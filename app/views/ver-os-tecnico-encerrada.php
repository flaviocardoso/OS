...
<div class="special go-botton row">
	...
	<div class="col-sm-2">
		<select ng-model="seach.dest_area" ng-areatecnico ng-options="option.valor as option.valor for option in areadados"></select>
		<label>Area</label>
	</div>
	<div class="col-sm-2">
...
<div>
	<table class="table" style="overscroll-behavior-x: contain;">
		<thead class="thead-dark">
			<tr>
				<th scope="col" class="text-center">procolo</th>
		...
		<tbody>
			<tr style="background-color:{{ordem.cor_os}}" ng-repeat="ordem in $ctrl.verOs | filter: { solicitante : seach.solicitante} | filter: {data_in : seach.data_in} | filter: {data_up : seach.data_up} | filter: {sol_coord : seach.sol_coord || undefined} | filter: {sol_setor : seach.sol_setor || undefined} | filter: {sol_dest_area : seach.dest_area || undefined} | filter:  {tecnico : seach.tecnico} | filter: {status : 'ENCERRADA'}">
				<td class="text-center">
					<a ui-sref="dashboard.os.verosinfotecnicoencerrada({ osID : ordem.id_os })" onclick="this.disabled=true;" ui-sref-active="active" class="btn btn-light">{{ordem.n_os}}</a>
				</td>
